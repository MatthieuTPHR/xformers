...
TORCH_LIBRARY_FRAGMENT(xformers, m) {
  ...
  m.def(TORCH_SELECTIVE_SCHEMA(
      "xformers::efficient_attention_forward_generic(Tensor query, Tensor key, Tensor value, bool compute_logsumexp, Tensor? attn_bias, float p, bool causal) -> (Tensor, Tensor)"));
  m.def(TORCH_SELECTIVE_SCHEMA(
